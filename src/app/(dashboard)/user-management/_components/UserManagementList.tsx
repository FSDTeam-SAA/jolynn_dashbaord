  ...
  const { data: response } = useQuery<UserListResponse>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/user?${params}`,
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="w-full sm:w-[170px]">
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              <SelectTrigger className="h-10 !w-full rounded-lg border-gray-200 bg-white text-sm font-medium text-gray-600 focus:ring-0">
                  ...
                  <tr key={user._id} className="hover:bg-slate-50/50">
                    <td className="py-4 pl-6 pr-4 font-semibold text-[#3b4cb8]">
                      <div className="flex items-center gap-3">
                        <div className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-[#2b3674]/10 bg-[#eef2ff] text-sm font-bold text-[#2b3674]">
                          {user.profilePicture ? (
                            <Image
                    ...
                    <td className="px-4 py-4 text-center">
                      <span
                        className={`inline-block min-w-[85px] rounded-full border px-3 py-1 text-center text-xs font-semibold capitalize ${getStatusStyles(user.status.toLowerCase())}`}
                      >
                        {user.status}
                      </span>
                    </td>
                    <td className="py-4 pl-4 pr-6">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className={`h-7 cursor-pointer rounded-md px-3 text-xs font-semibold text-white shadow-sm disabled:opacity-60 ${suspended ? "bg-[#22c55e] hover:bg-green-600" : "bg-[#dc2626] hover:bg-red-600"}`}
                        >
