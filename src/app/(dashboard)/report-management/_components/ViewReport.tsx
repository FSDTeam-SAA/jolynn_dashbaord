...
type ReportDetails = {
  ...
  helpWantedId?: { username?: string; email?: string; phone?: string; zipcode?: string; category?: string; message?: string };
  message: string;
  createdAt: string;
};
...
export default function ViewReport({ isOpen, onClose, reportId, reportType }: ViewReportProps) {
  ...
  const { data: response, isPending } = useQuery<{ success: boolean; message: string; data: ReportDetails }>({
  ...
  return <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="max-h-[85vh] w-[92%] max-w-[650px] overflow-y-auto gap-0 rounded-2xl border-0 bg-white p-6 shadow-2xl" overlayClassName="bg-slate-950/35 backdrop-blur-[3px]" showCloseButton={false} data-report-id={reportId}>
      <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-6"><DialogTitle className="text-xl font-bold text-gray-800">{isJobReport ? "Job Report Details" : "Help Wanted Report Details"}</DialogTitle><button type="button" onClick={onClose} aria-label="Close report details" className="rounded-md p-1 text-gray-700 hover:bg-slate-100"><X className="h-4 w-4" /></button></DialogHeader>
      ...
      {report && <div className="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-3">
        ...
        {isJobReport ? <><Detail label="Category" value={report.helpWantedId?.category || "N/A"} /><Detail label="Zipcode" value={report.helpWantedId?.zipcode || "N/A"} /></> : <><Detail label="Service" value={report.serviceId?.title || "N/A"} /><Detail label="Business Email" value={report.ownerId?.email || "N/A"} /></>}
