...
type JobReport = {
  _id: string;
  helpWantedId?: { username?: string; email?: string; phone?: string; category?: string; zipcode?: string; message?: string; userId?: string };
  userId: string;
  message: string;
  createdAt: string;
};
...
export default function ReportManagementList() {
  ...
  const [reportToDelete, setReportToDelete] = useState<(TableRow & { type: ReportType }) | null>(null);
  const [userToSuspend, setUserToSuspend] = useState<Pick<TableRow, "suspendUserId" | "suspendUserName"> | null>(null);
  const limit = 10;
  ...
  const activeError = isServiceReport ? reportsQuery.error : jobReportsQuery.error;
  ...
  const rows: TableRow[] = isServiceReport
    ? (reportsQuery.data?.data ?? []).map((item) => ({
        ...
        suspendUserName:
          ...
          [item.ownerId?.firstName, item.ownerId?.lastName].filter(Boolean).join(" ") ||
          item.ownerId?.email ||
          "this business owner",
      }))
      ...
      <div className="w-full overflow-x-auto rounded-xl border border-gray-100">
        <table className="w-full min-w-[850px] border-collapse text-left"><thead><tr className="bg-[#2b3674] text-[11px] font-semibold uppercase tracking-wider text-white"><th className="rounded-tl-xl py-3.5 pl-6 pr-4">Username</th><th className="px-4 py-3.5 text-center">Email</th><th className="px-4 py-3.5 text-center">Contact</th><th className="px-4 py-3.5 text-center">Report</th><th className="rounded-tr-xl py-3.5 pl-4 pr-6 text-center">Action</th></tr></thead>
          <tbody className="divide-y divide-gray-100 bg-white text-sm">
            {isLoading ? (
              <tr><td colSpan={5} className="py-10 text-center text-sm text-gray-500"><span className="inline-flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin text-[#2b3674]" />Loading reports...</span></td></tr>
            ) : activeError ? (
              <tr><td colSpan={5} className="py-10 text-center text-sm text-red-600">{activeError instanceof Error ? activeError.message : "Unable to load reports"}</td></tr>
            ) : rows.length ? (
              rows.map((item) => <tr key={item.id} className="hover:bg-slate-50/50"><td className="py-4 pl-6 pr-4 font-semibold text-[#3b4cb8]">{item.username}</td><td className="px-4 py-4 text-center text-gray-700">{item.email}</td><td className="px-4 py-4 text-center font-medium text-gray-700">{item.contact}</td><td className="max-w-[260px] px-4 py-4 text-center text-gray-600"><p className="truncate">{item.report}</p></td><td className="py-4 pl-4 pr-6"><div className="flex items-center justify-end gap-2"><button onClick={() => setSelectedReport({ id: item.id, type: activeTab })} aria-label={`View report from ${item.username}`} className="cursor-pointer rounded-md border border-[#2b3674]/25 p-1.5 text-[#2b3674] transition-colors hover:border-[#2b3674] hover:bg-[#eef2ff]"><Eye className="h-4 w-4" /></button>{item.email !== "N/A" && <a href={`mailto:${item.email}`} aria-label={`Email ${item.username}`} className="cursor-pointer rounded-md bg-[#2b3674] p-1.5 text-white hover:bg-[#20285f]"><Mail className="h-4 w-4" /></a>}<button type="button" disabled={!item.suspendUserId || suspendMutation.isPending} onClick={() => setUserToSuspend(item)} aria-label={`Suspend ${item.suspendUserName}`} title={item.suspendUserId ? `Suspend ${item.suspendUserName}` : "This post is not linked to a user account"} className="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-amber-200 px-2 py-1.5 text-xs font-semibold text-amber-700 transition-colors hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-40"><Ban className="h-3.5 w-3.5" />Suspend</button><button type="button" disabled={deleteMutation.isPending} onClick={() => setReportToDelete({ ...item, type: activeTab })} aria-label={`Delete report from ${item.username}`} className="cursor-pointer rounded-md border border-red-200 p-1.5 text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"><Trash2 className="h-4 w-4" /></button></div></td></tr>)
            ) : (
              <tr><td colSpan={5} className="py-8 text-center text-sm text-gray-500">No reports found</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <Pagination page={page} limit={limit} total={activeResponse?.meta.total ?? 0} currentCount={rows.length} onPageChange={setPage} disabled={isLoading} />
    </div>
...
async function fetchReports<T>(endpoint: ReportType, page: number, limit: number, accessToken?: string): Promise<ListResponse<T>> {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/${endpoint}?${params.toString()}`, { headers: { Authorization: `Bearer ${accessToken}` } });
  const data = await res.json().catch(() => ({}));
